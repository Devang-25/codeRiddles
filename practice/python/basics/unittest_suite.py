    ...
    def add_node(self, data, after=None, new_tail_end=False):
        ...
        if not after:
            if not new_tail_end:
                ...
                # tail = self.get_node(self.tail_end)
                # tail.next = Node(data)
                # new_tail = tail.next
                # # update tail to new node
                # self.tail = new_tail
                # # update tail end character
                # self.tail_end = data
...
class FormListCase(unittest.TestCase):
    '''
    Test Cases
    '''
    @classmethod
...
class AddNodesCase(unittest.TestCase):
    '''
    Test Cases
    '''
    LL = FormListCase.form_list()

    def test_add_nodes(self):
        self.assertEquals(self.LL.add_node(452, after=2), ['43', '2', '452', '-1'])
        self.assertEquals(self.LL.add_node(869, after=43), ['43', '869', '2', '452', '-1'])
        self.assertEquals(self.LL.add_node(99), ['43', '869', '2', '452', '99', '-1'])
        self.assertEquals(self.LL.add_node(3034, after=99), ['43', '869', '2', '452', '99', '3034', '-1'])
        self.assertEquals(self.LL.add_node(10, new_tail_end=True), ['43', '869', '2', '452', '99', '3034', '-1', '10'])

class GetNodesCase(unittest.TestCase):
    '''
    Test Cases
    '''
    LL = FormListCase.form_list()
    ...
    def test_get_node(self):
        assert self.LL.get_node('43').next.data == '2'

    def test_get_next_node(self):
        assert self.LL.get_next('43').next.data == '-1'

class InvalidListCase(unittest.TestCase):
    '''
    Test Cases
    '''
...
def suite():
    ...
    suite.addTest(FormListCase('test_form_list'))
    suite.addTest(AddNodesCase('test_add_nodes'))
    suite.addTest(GetNodesCase('test_get_node'))
    suite.addTest(GetNodesCase('test_get_next_node'))
    suite.addTest(InvalidListCase('test_tail_end_invalid_char'))
    return suite
