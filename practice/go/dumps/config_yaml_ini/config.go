...
type General struct {
	...
	TorSocksPortRange []int `yaml:"tor_socksport_range"`
...
func (cfg *Config) ValidateConfig(cfgData *ini.File) error {
	...
	cfg.GetIPConfigs()
	// cfg.Network.NumKeys = len(cfg.IPList)
	// cfg.Network.N = cfg.NumKeys - 1 // we assume client count to be 1
